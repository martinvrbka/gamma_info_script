...
def handle_host(host):
    ...
    try:
        ...
        sql_queries = "SELECT c.canisterindex, c.cntcode, max(r.refilldate) lastrefilldate FROM" \
                      " canister c JOIN refill r ON c.canisterid = r.canisterid" \
                      " GROUP BY c.canisterindex ORDER BY c.canisterindex;"

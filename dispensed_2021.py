...
def handle_host(host):
    ...
    try:
        ...
        sql_queries = "SELECT ch.channelindex, cntcode, round(SUM(IFNULL(volume, 0)), 2) totalvolume_ml FROM" \
                      " channel ch LEFT JOIN channel_action cha ON ch.channelid = cha.channelid AND" \
                      " cha.channelactiontypeid = 1 AND CAST(strftime('%Y', cha.action_date) AS INTEGER) == 2021" \
                      " GROUP BY ch.channelindex ORDER BY ch.channelindex;"

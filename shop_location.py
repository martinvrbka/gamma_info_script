...
def handle_host(host):
    ...
    try:
        ...
        # Get DB files
        db_files, _ = execute_command(ssh, "ls -a /home/ig/Deso/redlike/server/db/")

        sql_queries = "SELECT value FROM (SELECT value FROM pos_info WHERE key = 'name' UNION ALL SELECT value FROM" \
                      " pos_info WHERE key = 'city' UNION ALL SELECT value FROM pos_info WHERE key = 'address'" \
                      " UNION ALL SELECT value FROM pos_info WHERE key = 'address2') LIMIT 1;"

        # Loop over the databases and apply same SQLs for each of them if the file exists
        for path, password in [(r'local.db', r'QdfOfezP')]:
